...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        empty = set()
        self.a_levels.append(empty)
        for a in self.all_actions:
            # print('action = {0}'.format(a))
            a_node = PgNode_a(a)
            # print('action = {0}, a to obj = {1}, is sub = {2}'.format(a, a_node, a_node.prenodes.issubset(self.s_levels[level])))
            if a_node.prenodes.issubset(self.s_levels[level]):
                self.a_levels[level].add(a_node)
                self.add_action_level_inner(level, a_node)

    def add_action_level_inner(self, level, a_node):
        for s_node in self.s_levels[level]:
            if s_node in a_node.prenodes: # added based on suggestion from grader.
    ...
    def add_literal_level(self, level):
        ...
        empty = set()
        level_prev = level-1
        self.s_levels.append(empty)
        for a_level_node in self.a_levels[level_prev]:
            # print('a level = {0}'.format(a_level_node.effnodes))
            # Go through child nodes.
            for s_level_child in a_level_node.effnodes:
                # print('child = {0}'.format(s_level_child))
                a_level_node.children.add(s_level_child)
                s_level_child.parents.add(a_level_node)
                self.s_levels[level].add(s_level_child)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for a_eff_rem in node_a1.action.effect_rem:
            # print('{0}'.format(a_eff_rem))
            for a_eff_add in node_a2.action.effect_add:
                # print('a1 efffect rem = {0}, a2 efffect add = {1}'.format(a_eff_rem, a_eff_add))
                if a_eff_rem == a_eff_add:
                    return True
        for a_eff_rem in node_a2.action.effect_rem:
            # print('{0}'.format(a_eff_rem))
            for a_eff_add in node_a1.action.effect_add:
                # print('a2 efffect rem = {0}, a1 efffect add = {1}'.format(a_eff_rem, a_eff_add))
                if a_eff_rem == a_eff_add:
                    return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        effect_add_a1 = self.interference_mutex_helper(node_a1.action.effect_add,
                                           node_a2.action.precond_neg)
        effect_rem_a1 = self.interference_mutex_helper(node_a1.action.effect_rem,
                                           node_a2.action.precond_pos)
        effect_add_a2 = self.interference_mutex_helper(node_a2.action.effect_add,
                                           node_a1.action.precond_neg)
        effect_rem_a2 = self.interference_mutex_helper(node_a2.action.effect_rem,
                                           node_a1.action.precond_pos)
        if effect_add_a1 or effect_rem_a1 or effect_add_a2 or effect_rem_a2:
            return True
        return False

    def interference_mutex_helper(self, action_out, action_in):
        for a_out in action_out:
            for a_in in action_in:
                if a_out == a_in:
                    return True
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for a1 in node_a1.parents:
            for a2 in node_a2.parents:
                # print('something here? a1 = {0}, a2 = {1}'.format(a1, a2))
                if a1.is_mutex(a2):
                    return True
        return False
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        # print('umm = {0}'.format(node_s1.__eq__(node_s2)))
        # print('umm = {0}'.format(node_s1.children.__eq__(node_s2.children)))
        # print('umm = {0}'.format(node_s1.parents.__eq__(node_s2.parents)))
        # print('umm = {0}'.format(node_s1.symbol.__eq__(node_s2.symbol)))
        # print('umm = {0}'.format(node_s1.is_pos.__eq__(node_s2.is_pos)))
        if not node_s1.is_pos.__eq__(node_s2.is_pos) and node_s1.symbol.__eq__(node_s2.symbol):
            return True
        return False
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            ...
            for l, nodes in levels:
                # print('level # = {0}, nodes = {1}'.format(l, nodes))
                # for n in nodes:
                #     print(n.symbol)
                if goal_s in nodes:
                    level_sum += l
